...
# 从文件中逐行获取网址保存到列表，生成用于处理错误的"日期_TmpSave.txt"文件
def ReadFile():
    # 1.从命令行参数的文件中获取url，保存到Whisper中
    ...
    # 2.保存结果为临时文件"日期_TmpSave.txt"，如果产生中断，便于修复
    with open('{}_TmpSave.txt'.format(datetime.now().strftime("%Y%m%d")), 'w', encoding='utf-8') as f:
        for url in Whisper:
            f.write(url+' \n')

# Whisper的 url，抓取每个帖子的所需信息，结果存储在"日期_Others.txt"中
def ExcuteWhisper():
    if len(Whisper) != 0:
        # 结果存于"日期_Others.txt"中，追加写入
        ...
        # 评论有效（未被过滤）
            ...
            if re.match('沙发|板凳|[0-9]+楼', str) is not None:  
                ...
                if data_search is not None:
                    ...
                    last_comment_time = last_comment_time.replace(
                        '今天', datetime.now().strftime("%Y-%m-%d"))  # 替换'今天'为日期
